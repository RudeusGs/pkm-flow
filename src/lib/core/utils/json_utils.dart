...
Object? normalizeJsonKeys(Object? value) {
  ...
  if (value is Map) {
    return value.map((key, item) =>
        MapEntry(lowerFirst(key.toString()), normalizeJsonKeys(item)));
  }
  return value;
}
...
List<T> parsePagedItems<T>(Object? value, T Function(JsonMap) fromJson) {
  ...
  if (normalized is List) {
    return normalized.map((item) => fromJson(asMap(item))).toList();
  }
  final map = asMap(normalized);
  final items = map['items'] ?? map['data'];
  if (items is List) {
    return items.map((item) => fromJson(asMap(item))).toList();
  }
  return <T>[];
}
